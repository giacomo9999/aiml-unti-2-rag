...
const testEmbedding = [0, 1, 2];
const testPineconeQueryResult = [
  {
    id: 'sample-3',
    score: 0.54014945,
    values: [],
...
describe('queryOpenAIEmbedding', () => {
  ...
  it('should return an error if no userQuery is provided', async () => {
    await queryOpenAIEmbedding(req as Request, res as Response, next);
  ...
  it('should set res.locals.embedding if OpenAI returns a valid completion', async () => {
    res.locals!.userQuery = testUserQuery;
    ...
    expect(res.locals!.embedding).toEqual(testEmbedding);
...
describe('queryOpenAIChat', () => {
  ...
  it('should return an error if no userQuery is provided', async () => {
    res.locals!.databaseQueryResult = testPineconeQueryResult;
    await queryOpenAIChat(req as Request, res as Response, next);
  ...
  it('should return an error if no databaseQueryResult is provided', async () => {
    res.locals!.userQuery = testUserQuery;
    await queryOpenAIChat(req as Request, res as Response, next);
  ...
  it('should return an error if OpenAI does not return a completion', async () => {
    res.locals!.userQuery = testUserQuery;
    res.locals!.pineconeQueryResult = testPineconeQueryResult;
  ...
  it('should set res.locals.movieRecommendation if OpenAI returns a valid completion', async () => {
    res.locals!.userQuery = testUserQuery;
    res.locals!.pineconeQueryResult = testPineconeQueryResult;
    (mockOpenAI.chat?.completions.create as jest.Mock).mockResolvedValue({
      choices: [
    ...
    expect(res.locals!.movieRecommendation).toEqual(
